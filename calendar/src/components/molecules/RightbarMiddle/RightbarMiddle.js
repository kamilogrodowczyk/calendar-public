...
const RightbarMiddle = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(`${BASE_API_URL}/event/image/${clickedEvent.formattedDateToSort}`, fd).then((response) => {
        if (response.data.image) {
          update(response.data.image);
        } else {
          update('');
        }
        setImage('');
        ...
        <div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <AcceptElement isDisplayAnswer={isDisplayAnswer}>
              ...
              <DefaultButton type="submit">Ok</DefaultButton>
            </AcceptElement>
          </form>
        </div>
        {clickedEvent.image ? (
          <a href={clickedEvent.image} download style={style}>
